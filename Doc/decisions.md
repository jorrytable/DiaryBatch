...
| 4 | A | バックエンド（構造） | YouTube専用ロジックの汎用フック化 | `enrich.py`の`_fetch_oembed`が汎用関数のはずが`YOUTUBE_HOSTS`分岐を内包。ドメインごとの後処理フックのテーブル化が望ましいが優先度は低い。 |
| 5 | A | フロントエンド（構造） | `isVideoEmbed`のバックエンド重複 | `App.jsx`がYouTubeドメイン判定をsubstringで独自実装しており、`classify.py`/`enrich.py`のドメイン知識と3重管理。バックエンドが埋め込み種別をデータとして渡す設計に直せば解消するが、バックエンド側の変更を伴う。 |
| 6 | A | バックエンド（構造・見送り推奨） | Spotifyパス別分岐の一般化 | `classify.py`。現状Spotifyの1ケースのみのため、テーブル駆動化は時期尚早と判断。 |
| 7 | A | バックエンド（構造・見送り推奨） | `_extract_subtitle`の宣言的書き換え | `parser.py`。逐次的な文字列加工の積み重ねになっているが、コアなパース処理でリグレッションリスクがある。 |
| 8 | B | フロントエンド | タグバッジの表示色変更 | 現行の紫→グレーに変更。 |
| 9 | B | フロントエンド | 検索フィールドの各コントロールの位置整理 | 検索ボタンとリセット（条件をすべてクリア）ボタンの位置関係、統合検索ボックス内の「×」クリアボタンの扱い。 |
| 10 | B | フロントエンド | スマートフォン対応 | スマートフォン表示時に検索フィールド一式の高さを低くする。 |
| 11 | B | バックエンド | 分類ロジックの見直し | 「テキスト」「ラジオ」ジャンルへの対応ドメイン追加。`classify.py`の`DOMAIN_RULES`にエントリを増やす想定。 |
| 12 | B | バックエンド | レビュー本文の注（脚注）内容の取込 | 現状`parser.py`は`((...))`形式の脚注記号を除去するのみで中身は捨てている。中身自体を取り込む要否・取込先を要ヒアリング。 |
| 13 | B | バックエンド | もう1段深いレベルでの箇条書き記述の取込 | 現状`parser.py`は「- 」（作品名）「-- 」（感想）の2階層のみ対応。「--- 」等さらに深い階層の記法をどう解釈するか要ヒアリング。 |
| 14 | B | バックエンド | 初回読み込み時間の短縮 | `App.jsx`の`handleLogin`→検索API→DynamoDB全件取得の一連のレイテンシ。原因の切り分け・対応方針は未着手。 |
| 15 | B | フロントエンド | 各操作ボタン押下時にスクロール位置を最上部に戻す | 検索ボタン・条件をすべてクリアボタン・ページ送り（次へ/前へ）ボタンを押しても、一覧の途中にスクロールしたままになる。`App.jsx`の該当ハンドラ（`executeSearch`/`clearAllFilters`/`goToPrevPage`/`goToNextPage`）に`window.scrollTo`等の追加が必要と想定。 |
| 16 | B | フロントエンド | 埋め込みサービスごとのタグ抽出ロジック拡充 | Spotifyのポッドキャストは番組名、音楽はアーティスト名、noteはオーサー情報をそれぞれタグとして取り込み、カスタムタイトル（`:title=`）指定時はそのタイトル自体をタグにする。現状tagsはYouTubeのoEmbed応答（`author_name`）からのみ生成されており（`enrich.py`）、対象サービスごとの抽出処理拡充が必要。ユーザーの起票分類ではフロントエンド課題としているが、実装時は`batch/enrich.py`側（メタデータ取得ロジック）の変更が主体になる可能性が高い点に留意。 |
