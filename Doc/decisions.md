...
### 6.2 ユーザーからの追加起票（2026-08-02、着手は別途指示待ち）

**フロントエンド**
- タグバッジの表示色変更（現行の紫→グレーに変更）
- 検索フィールドの各コントロールの位置整理（検索ボタンとリセット（条件をすべてクリア）ボタンの位置関係、統合検索ボックス内の「×」クリアボタンの扱い）
- スマートフォン対応（スマートフォン表示時に検索フィールド一式の高さを低くする）

**バックエンド**
- 分類ロジックの見直し（「テキスト」「ラジオ」ジャンルへの対応ドメイン追加。`classify.py`の`DOMAIN_RULES`にエントリを増やす想定）
- レビュー本文における注（脚注）内容の取込（現状`parser.py`は`((...))`形式の脚注記号を除去するのみで中身は捨てている。中身自体を取り込む要否・取込先を要ヒアリング）
- レビュー本文におけるもう1段深いレベルでの箇条書き記述の取込（現状`parser.py`は「- 」（作品名）「-- 」（感想）の2階層のみ対応。「--- 」等さらに深い階層の記法をどう解釈するか要ヒアリング）
- パスワードを入力し初回読み込みを行う時間の短縮（`App.jsx`の`handleLogin`→検索API→DynamoDB全件取得の一連のレイテンシ。原因の切り分け・対応方針は未着手）

### 6.3 対応不要と判断してクローズした項目
