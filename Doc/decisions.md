...
| 12 | B | バックエンド | 初回読み込み時間の短縮 | **2026-08-02一次対応済み・デプロイ＆実測待ち**。`template.yaml`の`AuthorizerFunction`・`SearchFunction`に`MemorySize: 512`を追加（従来デフォルトの128MBはCPU割当が少なく、`json.dumps`/`gzip.compress`やLambdaコールドスタート自体に不利なため）。現存エントリ数1309件、体感は「ログインボタン押下後、何も表示されない時間が長い」（フロントの大量DOM描画のジャンクではなく、クリック〜データ到着までの区間が遅いと判断）。デプロイ後、CloudWatch Logsで`AuthorizerFunction`/`SearchFunction`の`Duration`/`Init Duration`を実測し、改善が乏しければペイロードサイズ削減（課題#1と関連）を次の一手として検討する。 |
