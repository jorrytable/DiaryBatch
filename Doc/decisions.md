...
- 出現件数の多い順に上位5件を候補としてドロップダウン表示し、候補をクリックした時だけ実際に一覧が絞り込まれる（入力中は絞り込まれない）
- 当初はIME確定（`compositionend`）ごとに候補計算していたが、「入力のたびに画面がリフレッシュされる」との指摘を受け、**Enterキー押下時にのみ候補計算する方式**に変更（`onKeyDown`で`isComposing`を判定し、変換確定のEnterと検索実行のEnterを区別）。入力中は表示値の更新のみで候補計算（全レビューのtags走査）は行わない
